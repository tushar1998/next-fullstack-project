...
interface DropDownRecursiveProps {
  items: MenuItem[];
}

const DropDownRecursive = ({ items }: DropDownRecursiveProps) => {
  return items.map((item, index) => {
      ...
      return (
        <DropdownMenuSub key={item?.id}>
          ...
          <DropdownMenuSubContent>
            <DropDownRecursive items={item?.subItems} />
          </DropdownMenuSubContent>
        </DropdownMenuSub>
      );
    }
    ...
    if (item) {
      const { id, menuTitle, shortcut, ...restItem } = item;

      return (
        <DropdownMenuItem key={id} {...restItem}>
          {menuTitle}
          <Conditional satisfies={shortcut}>
...
export interface DropDownProps extends DropdownMenuPrimitive.DropdownMenuProps {
  menu: { label: React.ReactNode; items: MenuItem[]; id: string };
}

const DropDown = ({ children, menu, ...props }: React.PropsWithChildren<DropDownProps>) => {
  return (
    <DropdownMenu {...props}>
